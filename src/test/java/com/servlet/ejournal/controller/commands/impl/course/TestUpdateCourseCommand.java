...
class TestUpdateCourseCommand {
    private static final UpdateCourseCommand command = new UpdateCourseCommand();
    ...
    void testExceptionParsingID() {
        when(reqMock.getParameter(COURSE_ID)).thenReturn("number");
        assertThrows(CommandException.class, () -> command.execute(reqMock));
    }

    @Test
    void testExceptionParsingDate() {
        when(reqMock.getParameter(COURSE_END_DATE)).thenReturn("01-01 2003");
        assertThrows(CommandException.class, () -> command.execute(reqMock));
    }

    @Test
    void testServiceException() throws ServiceException {
        when(serviceMock.updateCourse(any(FullCourseDTO.class))).thenThrow(ServiceException.class);
        assertThrows(CommandException.class, () -> command.execute(reqMock));
    }
}
